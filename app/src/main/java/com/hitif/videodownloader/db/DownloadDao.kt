...
interface DownloadDao {
    ...
    /** Update download state by URL (for HLS downloads that have no DownloadManager ID) */
    @Query("""
        UPDATE download_history
        SET state = :state, completedAt = :ts
        WHERE url = :url
    ...
    /** Update download progress by URL (for HLS downloads) */
    @Query("""
        UPDATE download_history
        SET progressBytes = :downloaded, totalBytes = :total, speedBps = :speed
        WHERE url = :url
    ...
    /** Find downloads stuck in DOWNLOADING/QUEUED state for too long */
    @Query("""
        UPDATE download_history
        SET state = 'FAILED'
        WHERE state IN ('DOWNLOADING', 'QUEUED')
          AND startedAt < :maxAge
    """)
    suspend fun failStaleDownloads(maxAge: Long)
}
